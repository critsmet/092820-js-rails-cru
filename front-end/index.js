...
//Click on an li of a city of a country and remove the li from the dom as well as that city from the database

countriesContainer.addEventListener('click', handleClick)

function handleClick(event){
  if(event.target.tagName === 'LI'){
    event.target.remove()
    // optimistic rendering
    fetch(`http://localhost:3000/cities/${event.target.dataset.id}`, {
      method: 'DELETE',
      headers: {
    ...
    .then(function(jsonMessageObj){
      if(jsonMessageObj.successful){
        event.target.remove()
...
function makeOneCityObjLI(cityObj){
  ...
  return `<li data-id=${cityObj.id}>${cityObj.name} - Population: ${cityObj.population}</li>`
}
